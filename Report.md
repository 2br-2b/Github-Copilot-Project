...
In my initial test, I received three assignments from Dr. Edward Kovach, a computer science professor at Franciscan University of Steubenville. I placed these assignments in a comment at the beginning of the program and had Copilot generate responses based off of these. I won't detail all three resopnses here, but they can be found [on Github](https://github.com/2br-2b/Github-Copilot-Project/tree/master/Scholarly%20Testing/).

In this paper, we will analyze the program for the first piece of homework.
...
I entered the first three lines myself. These were copied verbatum from the professor's assignment. I simply added the `# `s myself so that these were treated as comments by Copilot.
...
After entering three blank new lines, Copilot generated the `FMCLprog1` class all at once. Often, before generating a new class, Copilot will wait for three new lines, so this was expected.

This is an interesting implementation because Copilot, rather than simply receiving input and giving output, chose to define a `FMCLprog1` class and write the code in its `__init__` method. Then, later in the program, it calls `FMCLprog1()` which runs the `__init__` method. While this method works, I would personally do the simpler solution of:

```python
# How I would have structured Copilot's code
self.num1 = int(input("Enter a number: "))
self.num2 = int(input("Enter another number: "))
self.sum = self.num1 + self.num2
print("The sum of your numbers is", self.sum)
...
One thing to note about Copilot's response is that it does not properly follow the instructions. The instructions say that the program should "display those numbers with their sum", but Copilot only outputs the sum. The program is able to properly add the two numbers together, but does not display them properly.

---

> Copilot version: v1.7.3952
> VS Code version: 
> Version: 1.61.2
